...
def detect_disk_read_abuse(zone_path):
    ...
    """
    # Set the threshold for disk read abuse in bytes per second
    READ_THRESHOLD = 500000

    # Set the duration of the monitoring period in seconds

    # Get the total number of bytes used by the monitored folder at the beginning of the monitoring period
    disk_usage = psutil.disk_usage(zone_path)
    bytes_used_start = disk_usage.used

    # Monitor disk reads for the specified duration
    time.sleep(20)

    # Get the total number of bytes used by the monitored folder at the end of the monitoring period
    disk_usage = psutil.disk_usage(zone_path)
    bytes_used_end = disk_usage.used

    # Calculate the average number of bytes read per second during the monitoring period
    bytes_read_per_second = (bytes_used_end - bytes_used_start) / 20

    # Check if the average number of bytes read per second exceeds the threshold for disk read abuse
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: ./irondome.py [zone_path] [<extensions>]")
        sys.exit(1)
